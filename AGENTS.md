...
- `cargo nextest run --workspace --all-targets --all-features` matches the main CI test runner. `--all-features` is load-bearing: no feature is on by default, so omitting it silently skips every feature-gated suite.
...
- `cargo fmt --all -- --check` and `cargo clippy --workspace --all-targets --all-features -- -D warnings` must pass before PRs.
- `cargo doc --workspace --no-deps --all-features` checks rustdoc generation, matching docs.rs and CI.
- `cargo llvm-cov nextest --workspace --all-targets --all-features --lcov --output-path lcov.info` matches the coverage workflow when coverage changes matter.
